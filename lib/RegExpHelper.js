...
function buildPrefixParamExp(prefixs, _dataType, ignorePrefixs) {
    if (prefixs.some((p) => p.length > 1)) {
        ...
        if (_dataType === dataType.integer) return new RegExp(`(?:${starts})\\s*([0-9]+)`, "i");
        if (_dataType === dataType.mods) return new RegExp(`(?:${starts})\\s*([a-z0-9]+)`, "i");
        if (_dataType === dataType.date) return new RegExp(`(?:${starts})\\s*([0-9]{4}-[0-9]{1,2}-[0-9]{1,2})`, "i");
        return new RegExp(`(?:${starts})\\s*([^${ignores}]+)`, "i");
    }
    else {
        let starts = prefixs.join("");
        let ignores = ignorePrefixs.join("");
